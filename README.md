...
0s ────────────────── 3s ──── 3.1s ────────────────── 6s ────
│                      │        │                        │
│  🟠 橙色慢闪 (1Hz)   │  ⚫ 熄灭  │  🟢 绿色闪烁 (1Hz)     │  ⚫ 熄灭
│  (充电指示)           │  (过渡)   │  (USB有线模式提示)      │  (归还widget)
└──────────────────────┴─────────┴────────────────────────┘
```
...
| 阶段 | 时间 | 状态 | 灯光 | 频率 | 含义 |
|------|------|------|------|------|------|
| 1 | 0-3 秒 | `STATE_CHARGING_ORANGE` | 🟠 橙色慢闪 | 1Hz（500ms亮/500ms灭） | 充电指示（代表正在充电） |
| 2 | 3 秒整 | 状态切换 | ⚫ 熄灭 | - | 过渡（橙色熄灭，绿色尚未亮起） |
| 3 | 3-6 秒 | `STATE_USB_GREEN_BLINK` | 🟢 绿色闪烁 | 1Hz（500ms亮/500ms灭） | USB 有线模式提示 |
| 4 | 6 秒整 | `STATE_IDLE` | ⚫ 熄灭 | - | 归还 widget 控制 |

**代码逻辑：**
- 轮询间隔：100ms（1 tick = 100ms，30 ticks = 3 秒）
- 状态机：`STATE_CHARGING_ORANGE`（30 ticks）→ `STATE_USB_GREEN_BLINK`（30 ticks）→ `STATE_IDLE`
- 橙色和绿色**完全不会同时出现**，橙色熄灭后绿色才亮
- 切换状态时先调用 `set_led_color(0,0,0)` 熄灭，确保过渡干净

---

#### 场景 B：已充满（CHG=高 + USB 有）
...
| 阶段 | 时间 | 状态 | 灯光 | 含义 |
|------|------|------|------|------|
| 1 | 插线后 | `STATE_FULL_GREEN` | 🟢 绿色常亮 | 充满 / USB 有线模式 |
| 2 | 拔线后 | `STATE_IDLE` | ⚫ 熄灭 | 归还 widget 控制 |

**代码逻辑：**
- 状态机：`STATE_FULL_GREEN`（持续，只要 USB 插着且 CHG=高）
- 每 100ms 检测一次 CHG 状态，如果 CHG 变低则退出 `STATE_FULL_GREEN`
- 拔线时立即切换到 `STATE_IDLE`，熄灭 LED
- **绿色持续常亮，不会自动熄灭**，直到拔线或电池开始放电
